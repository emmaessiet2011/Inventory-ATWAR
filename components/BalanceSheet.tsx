...
const BalanceSheet: React.FC = () => {
  const { locations, suppliers, customers, products, payments, expenses, formatCurrency } = useGlobalContext();
  ...
  const totals = useMemo(() => {
    ...
    const closingStock = products.reduce((acc, p) => acc + ((p.stock || 0) * (p.unitPurchasePrice || 0)), 0);
    ...
    return {
      ...
      totalAssets: customerDue + closingStock + Math.max(0, accountBalances),
    };
  }, [suppliers, customers, products, payments, expenses]);
                  ...
                  <table className="w-full">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="px-6 py-4 text-slate-700 w-1/2 text-right">{formatCurrency(totals.accountBalances)}</td>
                      </tr>
                    </tbody>
                  </table>
                </td>
