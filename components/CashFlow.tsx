...
const CashFlow: React.FC = () => {
  const { payments, expenses, locations, formatCurrency } = useGlobalContext();
  ...
  const transactions = useMemo<CashTx[]>(() => {
    const paymentRows: CashTx[] = payments.map(p => ({
      id: `PAY-${p.id}`,
      date: p.date,
      account: p.account || 'Unspecified',
      location: '',
      description: `${p.contactType} payment - ${p.contactName}`,
      method: p.method,
      details: p.referenceNo || '',
      type: p.type === 'received' ? 'Inflow' : 'Outflow',
      amount: p.amount || 0,
    }));

    const expenseRows: CashTx[] = expenses.map(e => ({
      id: `EXP-${e.id}`,
      date: e.date,
      account: e.paymentAccount || 'Expense',
      location: e.location || '',
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter(tx => {
      ...
      const matchesDate = Number.isFinite(startMs) && Number.isFinite(endMs) ? (txTime >= startMs && txTime <= endMs) : true;
      const matchesAccount = accountFilter === 'all' || tx.account === accountFilter;
      const matchesLocation = locationFilter === 'all' || tx.location === locationFilter;
      ...
      return matchesDate && matchesAccount && matchesLocation && matchesType && matchesSearch;
    });
  }, [transactions, rangeStart, rangeEnd, accountFilter, locationFilter, typeFilter, search]);

  const accountOptions = useMemo(
    () => Array.from(new Set(transactions.map(tx => tx.account))).filter(Boolean).sort(),
    [transactions]
  );

  const summary = useMemo(() => {
    const inflow = filtered.filter(tx => tx.type === 'Inflow').reduce((acc, tx) => acc + tx.amount, 0);
    const outflow = filtered.filter(tx => tx.type === 'Outflow').reduce((acc, tx) => acc + tx.amount, 0);
    let running = 0;
    const withBalance = filtered.map(tx => {
      running += tx.type === 'Inflow' ? tx.amount : -tx.amount;
      return { ...tx, runningBalance: running };
    });
    return { inflow, outflow, withBalance };
  }, [filtered]);
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-white">
          <div className="flex flex-wrap gap-2">
            <button className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-1.5 shadow-sm transition-colors">
          ...
          <table className="w-full text-sm text-left border-collapse">
            <thead className="bg-white text-slate-900 font-bold border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 whitespace-nowrap">Debit</th>
