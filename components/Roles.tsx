...
import { 
  ...
  ChevronRight, Lock, Key, Info, CheckCircle2,
  Users, UserCog, ShieldAlert, X, Save, 
  Check, Square, CheckSquare, Layers, 
  ...
  Home, Landmark, DollarSign
} from 'lucide-react';
import { useGlobalContext, Role } from '../src/context/GlobalContext';
...
const PERMISSION_MODULES: PermissionGroup[] = [
    ...
    { module: 'Sell', icon: Tags, permissions: ['View all sell', 'View own sell only', 'View paid sells only', 'View due sells only', 'View partially paid sells only', 'View overdue sells only', 'Add Sell', 'Update Sell', 'Delete Sell', 'Commission agent can view their own sell', 'Add sell payment', 'Edit sell payment', 'Delete sell payment', 'Edit product price from sales screen', 'Edit product discount from Sale screen', 'Add/Edit/Delete Discount', 'Access all sell return', 'Access own sell return', 'Add edit invoice number'] },
    ...
    { module: 'Order', icon: Truck, permissions: ['View order', 'Add order', 'Edit order', 'Delete order'] },
...
const Roles: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
    const { roles: globalRoles, addRole, updateRole, deleteRole } = useGlobalContext();
    ...
    const [roles, setRoles] = useState<Role[]>(globalRoles);
    ...
    const handleOpenModal = (role: Role | null = null) => {
        ...
        // Initialize empty permissions or existing ones
        const initialPerms: Record<string, string[]> = {};
        PERMISSION_MODULES.forEach(m => initialPerms[m.module] = []);
    ...
    const handleSaveRole = () => {
        ...
        const totalPermissions = (Object.values(selectedPermissions) as string[][]).reduce((acc, curr) => acc + curr.length, 0);
        ...
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Define access levels and functional permissions for user groups.</p>
                </div>
                <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredRoles.map(role => (
                            <div 
                                ...
                                className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all group relative cursor-pointer"
                            >
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">
                                        <Key size={14} /> {role.permissionsCount} Permissions
                                    </div>
                                </div>
                            </div>
                        ))}
